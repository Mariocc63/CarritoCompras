...
exports.crearEstado = async (req, res) => {
  ...
  const existeestado = await sequelize.query(
    "select * from estados where nombre = :estado",
    {
      replacements: {
        estado,
      },
  ...
  if (existeestado.length > 0) {
    ...
    try {
      await sequelize.query("EXEC InsertarEstado :estado", {
        replacements: { estado },
...
exports.actualizarEstado = async (req, res) => {
  ...
  try {
    await sequelize.query("EXEC ActualizarEstado :idestados, :estado", {
