...
const validationSchema = yup.object().shape({
  ...
  fecha_entrega: yup
    ...
    .min(new Date(), "La fecha de entrega debe ser hoy o una fecha futura")
    .typeError("La fecha de entrega debe ser válida"),
});
            ...
            <ul>
              {cart.map((item) => (
                <li key={item.producto.codigo}>
                  {item.producto.producto} - Cantidad: {item.cantidad} - Precio:
                  Q{item.producto.precio.toFixed(2)} - Subtotal: Q
      ...
      <Dialog open={openDialog} onClose={handleCloseDialog}>
        ...
        <DialogContent>
          <DialogContentText>
            ...
            <br />
            Nombre Completo: {shippingData.nombre_completo}
            <br />
            Dirección: {shippingData.direccion}
            <br />
            Teléfono: {shippingData.telefono}
            <br />
            Correo Electrónico: {shippingData.correo_electronico}
            <br />
            Fecha de Entrega:{" "}
            {moment(shippingData.fecha_entrega).format("DD/MM/YYYY")}
            <br />
