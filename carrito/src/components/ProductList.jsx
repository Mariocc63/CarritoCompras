...
import { Box, Typography, Card, CardContent, CardMedia, Grid, Button, IconButton, AppBar, Toolbar, Menu, MenuItem, Badge } from '@mui/material';
...
const ProductList = ({ token }) => {
  ...
  return (
    <Box sx={{ paddingTop: '70px' }}> 
      ...
      <Typography variant="h4" align="center">Lista de Productos</Typography>
        ...
        {products.productos.map(product => (
          <Grid item key={product.codigo} xs={12} sm={6} md={4}>
            <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
              ...
              />
              <CardContent>
                <Typography variant="h6" gutterBottom>{product.producto}</Typography>
                <Typography variant="body2" >Código: {product.codigo}</Typography>
                <Typography variant="body2" >Marca: {product.marca}</Typography>
                <Typography variant="body1">Precio: Q{product.precio.toFixed(2)}</Typography>
              </CardContent>
              <Button 
                variant="contained" 
                color="primary" 
